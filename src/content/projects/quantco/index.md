...
The ambiguity wasn't about who we were designing for — claim handlers were accessible, and I had six of them in weekly sessions. It was about what we were building. The capability arrived before the use case: we could extract invoice and quote line items and score how closely they matched, but nobody had settled what a claim handler should *do* with a score. The engineers' question was how to expose the model's output. The more useful question was what decision it was meant to serve.

We also didn't own the workflow we were entering. The system of record was a third-party application and our surface was read-only, so every insight we surfaced had to hand off cleanly to a tool we didn't control. The workflow already existed, with its own habits, approval paths, and lines of accountability. We weren't designing a workflow, we were designing an intervention into one.

So the work became locating the few moments where a model's output could genuinely change a decision, and designing those moments to hold. Each relationship we surfaced — starting with invoice to quote — had to be legible on its own terms and compose with the ones the automation roadmap would add next. This modular approach let the product absorb new model outputs without redrawing the interface each time, and let a read-only assistive layer become the foundation for the automation it was building toward.
